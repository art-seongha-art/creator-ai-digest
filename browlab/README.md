...
3. **모양**: 눈썹마다 조절점 7개 — 머리·중간·아치의 윗선/아랫선 + 꼬리. 윗선 점은 윗선을, 아랫선 점은 아랫선을 움직이고, 꼬리 점은 길이와 각도, 몸통을 끌면 통째로 이동. 두 손가락으로 확대, 빈 곳을 끌면 화면 이동, **빈 곳을 톡 치면 전/후 전환**. `대칭` 이 켜져 있으면 반대쪽이 거울처럼 따라옵니다. 미세 조정 버튼과 화살표 키는 0.5 mm(Shift 2 mm) 단위. 잘못 움직였으면 `↶`(Ctrl+Z) · `↷`(Ctrl+Shift+Z).
