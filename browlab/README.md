...
3. **모양**: 눈썹마다 조절점 9개 — 머리·중간·아치·꼬리앞의 윗선/아랫선 + 꼬리. 윗선 점은 윗선을, 아랫선 점은 아랫선을 움직이고, 꼬리 점은 길이와 각도, 몸통을 끌면 통째로 이동. 두 손가락으로 확대, 빈 곳을 끌면 화면 이동, **빈 곳을 톡 치면 전/후 전환**. `대칭` 이 켜져 있으면 반대쪽이 거울처럼 따라옵니다. 미세 조정 버튼과 화살표 키는 0.5 mm(Shift 2 mm) 단위. 잘못 움직였으면 `↶`(Ctrl+Z) · `↷`(Ctrl+Shift+Z).
   - 무대 위 버튼: `원본`(누르는 동안 원본) · `비교`(가운데 선을 끌어 좌우 반씩 전/후) · `점`(조절점 감추기) · `가이드선`(콧방울 기준 디자인선) · `대칭` · `눈썹`/`전체`(확대) · `잔털 지우기`.
   - 아이패드 + 펜슬: 손바닥이 닿아도 획이 끊기지 않습니다(펜이 제스처를 소유). 손가락 둘은 확대.
   - 확대·축소는 **사진에만** 적용됩니다. 메뉴·패널 위에서 손가락을 벌려도 화면이 커지지 않습니다(상담 스튜디오가 열려 있는 동안만 그렇고, 갤러리나 다른 화면에서는 평소대로 확대됩니다).
   - `⛶` 전체화면은 **페이지 자체의 전체화면**이라 아이패드에서 스와이프로 풀리지 않습니다. 상단 회원명 줄만 물러나고 **도안·색·저장 패널은 그대로 남습니다**(폰에서는 탭 줄로 접히고, 탭을 누르면 다시 펴집니다). 끝내려면 무대의 `전체화면 끝내기` 또는 Esc. 상담용으로 상시 쓰신다면 아이패드에서 **공유 → 홈 화면에 추가** 로 설치하면 Safari 화면 없이 앱처럼 열립니다.

3-1. **잔털 지우기**: 지름 0.5~4 mm(기본 2 mm) 브러시로 잔털·잡티 위를 문지르면, **문지른 자리가 주변의 깨끗한 피부로 바뀝니다**(포토샵 스팟 힐링과 같은 방식). 모공·피부 결이 그대로 살아 있어 뭉갠 자국이 남지 않습니다. 문지르는 동안 바로 지워지는 게 보입니다. 획 단위로 `↶` 되돌리기, `모두 복구` 로 전체 취소. 지운 자국은 상담과 함께 저장되어 다시 열 때 그대로 재생됩니다.
4. **색·농도**: 강도(색소 농도), 부드러움(피부 속에 자리 잡은 느낌), **선 진하기(1 = 그린 그대로, 아래는 연하게 위는 진하게)**, **선 굵기(5 = 그린 그대로)**, 색소 6종 + **머리색 자동**, 밝기·따뜻함. 경과 미리보기 `시술 직후 / 치유 후(4주) / 1년 후` 는 강도·부드러움 프리셋입니다(치유 후 30~50% 연해지는 일반적 경과 기준).
5. **저장** — 두 가지가 다릅니다.
   - **조절한 내용은 저절로 저장됩니다**(상담 한 건당 하나, 계속 덮어씀). 도안·조절점·색·메모·지운 잔털까지. 갤러리에서 `상담 이어하기` 를 누르면 이 상태 그대로 열립니다.
   - **`시안 한 장 저장` 은 새로 저장입니다.** 누를 때마다 지금 화면이 **한 장씩** 갤러리에 쌓입니다(같은 회원명이면 `이서연`, `이서연 2`, `이서연 3` …). 여러 시안을 남겨 두고 상담자와 고르는 용도입니다. 덮어쓰지 않습니다.
   - 그 밖에: 치수(길이·두께·꼬리 높이·아치 높이·미간을 **좌우 각각과 차이**로, 동공 간 거리 기준 mm), 상담 메모, `내려받기`, `1:1 출력 시트`(저장한 시안으로 A4 시트 작업), `전후 비교로 한 장`(눈썹 부분을 잘라 원본·시안을 나란히). 갤러리에 저장한 시안은 눌러서 원본과 반씩 비교하거나 눌러서 전/후를 볼 수 있습니다. `원본` 버튼(또는 스페이스)을 누르고 있는 동안 원본 사진.

### 짧은 주소

`https://art-psh.com/BLAB` (대소문자 무관, `/blab` 도 됨) → BrowLab 으로 이동합니다. 포트폴리오 저장소(`art-seongha-art/portfolio`)의 `BLAB/index.html` 이동 페이지이고, `/deck`(강의 허브)와 같은 방식입니다. 주소창은 이동 후 4090 주소로 바뀝니다.
