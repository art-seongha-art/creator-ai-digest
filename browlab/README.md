...
3-1. **잔털 지우기**: 지름 0.5~4 mm 브러시로 잔털·잡티 위를 문지르면, 자국 안에서 **주변 피부보다 어두운 화소만** 주변 피부색으로 메웁니다(피부 결은 남습니다). 획 단위로 `↶` 되돌리기, `모두 복구` 로 전체 취소. 지운 자국은 상담과 함께 저장되어 다시 열 때 그대로 재생됩니다.
