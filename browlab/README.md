...
5. **저장**: 치수(길이·두께·꼬리 높이·아치 높이·미간을 **좌우 각각과 차이**로, 동공 간 거리 기준 mm), 상담 메모, `갤러리에 저장`(회원명, 비우면 회원_N), `내려받기`, `1:1 출력 시트`(저장본으로 A4 시트 작업), **전후 비교 이미지**(눈썹 부분을 잘라 원본·시안을 나란히). 조절 상태는 자동 저장되어 갤러리의 `상담 이어하기` 로 그대로 열리고, 갤러리에 저장한 시안은 눌러서 원본과 반씩 비교하거나 눌러서 전/후를 볼 수 있습니다. `원본` 버튼(또는 스페이스)을 누르고 있는 동안 원본 사진.
