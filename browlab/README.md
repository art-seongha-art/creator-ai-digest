...
6. 마스크 둘레 피부톤을 원본에 맞춘 뒤(`--no-tone-match` 로 생략) **눈썹 영역만 원본 타일에 합성**하고, 그 타일을 **원본 사진의 제자리에 되붙입니다** → `NN_스타일_색_composited.png`. 마스크 밖은 원본 사진과 픽셀 단위로 같습니다.
