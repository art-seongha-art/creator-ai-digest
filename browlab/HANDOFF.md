...
### 3.2.31 조절점 7개 · 비교 슬라이드 · 되돌리기 · 잔털 지우기 · 아이패드 (2026-09-13)

- **조절점 7개**: 머리·중간·아치 × 윗선/아랫선 + 꼬리. 모델을 바꿈 — 두께 배율(k) 보간이 아니라 **"도안 자체의 윤곽선(기준) + 조절점 변위"**. 등뼈(머리·아치·꼬리 2차 곡선)에 도안의 매끄러운 윗선/아랫선을 그대로 놓은 것이 기준이고, 각 조절점은 그 기준선에서의 변위. 변위를 비균등 Catmull-Rom(매듭 uH·uM·uA·1, 꼬리는 0)으로 섞어 윗선/아랫선을 만들고, 메시는 그 선을 도안 폭 비율(eT/gT)로 넘겨 그린다. 손대지 않으면 변위 0 → 도안 그대로. **조절점이 항상 손가락 밑에 있음** — 전 모델은 거리→두께로 바꾸고 법선 위에 다시 그려서 옆으로 끌면 점이 손가락에서 벗어났고, 아이패드(펜·터치)에서 "끊긴다/제대로 안 된다"는 보고의 한 원인.
- 아이패드 대응 나머지: `pointermove` 마다 렌더하던 것을 **프레임당 한 번**(`dzRenderSoon`, rAF)으로 — 펜슬은 240Hz 라 이벤트가 렌더보다 빨라 밀렸음. 화면이 1600px 보다 작을 땐 **1600px 축소본**을 그림(원본 3000px 매 프레임 스케일링 제거). 터치 포인터의 조절점 반경 30px(펜·마우스 18). `-webkit-touch-callout:none`.
- **비교**(반씩 슬라이드): 선 오른쪽만 시안(`paintBrows(clipX)`), 선·손잡이·전/후 라벨은 화면 좌표로. 원본 버튼은 누르고 있는 동안, **빈 곳을 톡 치면**(6px·450ms 이내, 핀치 아님) 전후가 토글. 갤러리에 저장한 시안은 서버가 `compare:{before:photo.jpg}` 와 `edit:{overlay:100}` 을 붙여 라이트박스의 슬라이드/홀드가 그대로 됨(`side_by_side` 저장본 제외).
- **되돌리기/다시**: 상태 스냅샷(조절점·색·도안·대칭·잔털 획) 스택 120개. 드래그 끝·미세조정·도안 전환·슬라이더 `change`·프리셋·색·대칭·복사·초기화·획 후 push. Ctrl/Cmd+Z, Shift 로 다시, Ctrl+Y. 화살표 키 0.5mm(Shift 2mm).
- **잔털 지우기**(스팟 힐링, `healStroke`): 브러시(지름 0.5~4mm) 자국 안에서 **주변 피부보다 어두운 화소만**(max-filter→box-blur 로 피부 밝기 추정, 10 이상 어두우면 털, 1px 팽창) 털이 아닌 화소의 가우시안 가중 평균으로 메움. 피부 결은 남고 털만 사라짐. 획(반지름·점열)만 상태에 저장, 열 때·되돌릴 때 재생. 재생한 `base` 는 **바뀐 영역(bbox)만** 사진 위에 그림 — 전체를 다시 그리면 캔버스 리샘플링 차이로 화면 전체가 0.3% 밝아져 전후 비교 픽셀 테스트가 깨졌음(눈에는 안 보이지만 원인 불명의 차이는 남기지 않기로).
- 그 밖에: 가이드선(콧방울→안쪽 눈꼬리/홍채 바깥/바깥 눈꼬리 + 눈선), 점 감추기 버튼, 패널 접기(폰), 전체화면(Fullscreen API, 안 되면 접기), 전후 비교 이미지(눈썹 띠 잘라 좌우 나란히, 내려받기/갤러리 저장), 좌우 치수와 차이, 선 진하기 1·선 굵기 5 = 그대로(아래는 연하게/얇게, 위는 진하게/굵게 — 알파 거듭제곱과 1px 팽창/침식 보간; 예전 상태의 0~100·1~3 눈금은 열 때 환산).
- **아이패드에서 "끊긴다"의 진짜 원인 3가지** (연구자 보고, M1 iPad + 펜슬):
  1. **손바닥**. 펜슬로 끄는 중에 손바닥이 닿으면 `pointerdown`(pointerType `touch`)이 하나 더 들어와 `ptrs.size===2` → 핀치로 오인 → `drag=null`. 즉 한 획마다 끊김. 이제 **펜이 제스처를 소유**: 펜 드래그 중 touch 포인터는 무시, 드래그를 시작한 포인터 id 만 움직이고 끝낼 수 있음(손바닥이 떨어져도 획 유지). 손가락 둘은 여전히 핀치.
  2. **조절점이 손가락에서 벗어남** — 위의 기준선+변위 모델로 해결.
  3. `pointermove` 마다 렌더 → 펜슬 240Hz 에서 밀림. rAF 1프레임 1렌더 + 1600px 축소본.
  - 더불어 `setPointerCapture` 를 try/catch 로 감쌈(캡처 실패가 제스처 전체를 날리지 않게), 터치 히트 반경 30px, `-webkit-touch-callout:none`.
  - **네이티브 앱은 필요 없음**: 위 셋은 전부 Pointer Events 처리 문제였고, 아이패드 Safari 가 못 하는 것은 없었음.
- 전체화면 버튼(Fullscreen API, iPhone 처럼 지원 안 하면 패널 접기로 폴백). 폰에서는 무대 위 도구 버튼이 한 줄로 가로 스크롤(두 줄로 접히면 사진 높이를 먹음).
- 검증: e2e 44항목 + 펜/손바닥/핀치 6항목 통과(중간 점 두께 변화, Ctrl+Z/↷, 비교 좌우 픽셀, 점 톡 토글, 획 후 밝기 상승·되돌리기·재생, 저장본 라이트박스 비교, 손바닥 무시, 두 손가락 핀치 유지 등).

### 3.2.32 연습용 얼굴 생성 시 HTTP 502 — 리스트를 dict 키로 쓴 것 (2026-09-13)

- 증상: 연습용 얼굴 생성을 누르면 **502**. 이어서 **서버 업데이트도 안 됨**.
- 원인 하나: `title_ko()` 의 `P.BROW_CONDITIONS.get(p.get("brow_condition", ""))`. 3.2.27 에서 눈썹 조건을 **여러 개 고를 수 있게** 하면서 `shown["brow_condition"]` 이 2개 이상일 때 **리스트**가 되는데, `dict.get(list)` 는 `TypeError: unhashable type: 'list'`. `do_POST` 는 `BadRequest` 와 파이프 오류만 잡고 있어 **핸들러 스레드가 그대로 죽음 → 응답 없이 연결 종료 → Tailscale Funnel 이 502**. 작업 자체는 `submit()` 에서 이미 만들어졌으므로 생성은 돌아가고 화면만 502.
  - 같은 함수를 `gallery()` 와 `GET /api/jobs` 도 부르므로, 그런 작업이 목록에 있는 동안 **갤러리·작업 목록도 502**. 그래서 화면이 전반적으로 먹통.
  - 그 작업이 `queued/running` 으로 남아 `POST /api/update` 의 "진행 중인 작업" 가드에 걸림 → **서버 업데이트도 거부**. 두 증상이 한 뿌리.
- 고침: `_key()` 헬퍼(리스트면 첫 값, 문자열 아니면 빈 문자열)로 모든 프리셋 조회를 감싸고, 눈썹 조건은 "모량 부족(듬성듬성) 외 2가지" 로 표기. **그리고 `do_GET`/`do_POST` 에 `except Exception` → 500 JSON + stderr 트레이스백**(`fail_500`). 응답 없이 끊는 일이 다시는 없도록. 업데이트는 409 일 때 페이지가 "그래도 할까요?" 를 물어 `force:true` 로 재시도.
- **테스트가 놓친 이유**: `test_07b` 는 `build_argv` 만 직접 불렀고 `summary()`/`title_ko()` 를 거치지 않았음. 회귀 테스트 두 개 추가 — 여러 조건으로 **실제 POST → 201 + 갤러리/작업목록 200**, 그리고 핸들러가 예외를 던져도 **500 으로 답하고 서버가 계속 산다**.
- 교훈: 파라미터를 단일값→다중값으로 넓힐 때는 **그 값을 읽는 모든 곳**(표시용 제목 포함)을 같이 훑을 것. 그리고 HTTP 핸들러에 만능 예외 처리가 없으면 사용자는 원인 없는 502 만 본다.

### 3.2.33 슬라이더가 "바로바로" 안 되던 이유 · 전체화면이 풀리던 이유 (2026-09-13)

- **슬라이더 지연**: 슬라이더 `input` 마다 (a) `retint()` 가 도안 두 장을 **픽셀 단위로 전부 다시 칠하고**(41k×2, 선 굵기가 5가 아니면 3×3 이웃까지), (b) `dzRender()` 를 **동기로** 불러 사진 + 메시 320 삼각형을 다시 그렸음. 아이패드는 `input` 이 프레임보다 훨씬 자주 남.
  - 고침 1: `retint()` 를 **모양(선 굵기·진하기)과 색으로 분리**. 모양은 `shapeCanvas(side,wgt,dens)` 로 캐시(최대 24개), 색은 `source-in` 합성 한 번. 실측(같은 브라우저): 색 슬라이더 **10ms대 → 0.04ms**, 선 굵기 재방문 **0.03ms**.
  - 고침 2: 칠해 둔 **눈썹 레이어를 캐시**. 키 = 조절점·도안·뷰·blur·색·굵기·진하기. 강도/부드러움/비교선/전후 토글은 레이어를 다시 그리지 않음. 실측: 강도 슬라이더 **13.3ms → 0.53ms**, 비교선 드래그 **13.3ms → 0.69ms**. 조절점 드래그는 여전히 13ms(레이어를 실제로 다시 그려야 하므로).
  - 고침 3: 슬라이더도 `dzRenderSoon()`(rAF) 경유.
  - `dzExport()` 는 화면 레이어를 잠시 비켜 두고 전체 해상도로 따로 칠함 — 캐시가 저장본을 오염시키지 않는지 테스트로 고정(warm/cold 결과 동일).
- **전체화면이 살짝만 내려도 풀림**: 아이패드 Safari 는 요소 전체화면에서 **아래로 스와이프하면 OS 가 해제**. 그래서 전체화면을 **페이지 자신의 모드**(`.dz.immersive`: 상단바·패널 숨김, 무대에 "전체화면 끝내기" 버튼)로 바꾸고, 브라우저 전체화면은 **덤으로** 같이 요청. OS 가 제 것을 풀어도 우리 것은 남음(`fullscreenchange` 에서 해제하지 않음). Esc 는 전체화면만 빠져나오고 스튜디오는 유지 — 같은 `document` 의 두 keydown 리스너가 모두 도는 바람에 Esc 한 번에 창까지 닫히던 것을 `stopImmediatePropagation()` 으로 막음(먼저 등록된 쪽이 이긴다).
  - 근본 해결책으로 `apple-mobile-web-app-capable` 등 메타를 추가: 아이패드에서 **공유 → 홈 화면에 추가** 로 설치하면 Safari UI 자체가 없는 앱으로 뜸(제스처로 되돌아갈 브라우저가 없음). 네이티브 앱은 여전히 불필요.
  - **첫 판은 틀렸음**: 전체화면에서 상단바와 **작업 패널을 둘 다** 숨겼더니 도안·색·저장에 손이 닿지 않아 "더 이상해졌다"는 보고. 사진만 크고 아무것도 못 하는 화면은 전체화면이 아님. 지금은 **상단바(회원명 줄)만** 물러나고 패널은 남으며, 폰에서는 패널이 탭 줄로 접힘(탭을 누르면 다시 펴짐). 아이패드 실측 +7%(상단바), 실제 기기에서는 Safari 툴바까지 사라져 +20% 남짓. "전체화면 끝내기" 는 도구 줄 맨 앞(`order:-1`).
- 검증: 성능 테스트 7항목(위 수치 + 저장본 동일성), 전체화면 10항목(아이패드·폰 각각 면적 증가, 종료 버튼, OS 해제 후 유지, Esc 두 단계), e2e 44항목, 파이썬 120개.

### 3.2.34 두 가지 저장을 구분해서 보여주기 (2026-09-13)

- 연구자 질문: "저장은 움직일 때마다 되는 거잖아? 갤러리에 저장은 사실 **새로 저장**에 가깝지?" — 맞음. 코드가 실제로 그 둘이었는데 화면이 구분해 주지 않았음.
  - `POST api/designs/<id>` (900ms 디바운스): 상담 한 건의 **작업 상태**(도안·조절점·색·메모·잔털 획) — 하나뿐이고 계속 덮어씀. `상담 이어하기` 가 읽는 것.
  - `POST api/saves`: 지금 화면을 **새 파일**로 굽는 스냅샷. 누를 때마다 갤러리에 한 장씩 쌓임.
- 화면: 저장 탭 맨 위에 "조절한 내용은 저절로 저장됩니다 … · 저장 중…/저장됨/저장 실패" 줄(`#dzAuto[data-state]`), 그 아래 "시안 남기기 — 누를 때마다 지금 화면이 한 장으로" 와 **`시안 한 장 저장`**(전 `갤러리에 저장`), 그리고 남긴 시안 목록(번호·이름·시각).
- 같은 회원명으로 다시 누르면 서버가 **같은 상담 안에서 번호를 붙임**(`이서연` → `이서연 2` → `이서연 3`). 전에는 갤러리에 같은 이름 카드가 셋 나란히 떠서 어느 게 어느 건지 알 수 없었음.
- 검증: 서버 테스트(번호 매기기·덮어쓰지 않음·갤러리 라벨 4종), 브라우저 7항목(자동 저장 상태 표시, 두 번 눌러 두 장, 목록 표기, 갤러리 반영).

### 3.2.35 메뉴는 확대되지 않고 사진만 확대되게 (2026-09-13)

- 증상: 아이패드에서 패널 위에 손가락 둘을 벌리면 **브라우저가 페이지 전체를 확대** — 메뉴 글씨가 커지고 레이아웃이 틀어짐. 무대(`.dz-stage`)는 `touch-action:none` 이라 우리 핀치가 먹지만, 그 밖은 Safari 의 페이지 줌.
- `user-scalable=no` 는 iOS 10 이후 무시됨. 실제로 듣는 것은 **Safari 의 `gesturestart`/`gesturechange`/`gestureend` 를 `preventDefault()`** 하는 것. 스튜디오가 열려 있는 동안만 막음 — 갤러리·폼에서는 브라우저 확대를 그대로 둬야 접근성이 유지됨.
- 함께: `.dz{touch-action:manipulation}`(더블탭 줌 없음), `.dz-side/.dz-body{touch-action:pan-y}`(스크롤은 되고 핀치는 안 됨), 데스크톱의 `Ctrl+휠`·`Ctrl +/-/0` 도 스튜디오 안에서는 무시.
- 무대는 그대로: 휠 확대, 두 손가락 핀치, `눈썹`/`전체` 버튼.
- 검증 6항목: 패널·무대·탭 어디서 시작해도 페이지 줌 거부 / 스튜디오를 닫으면 다시 허용 / 패널 위 Ctrl+휠 거부 / 무대 휠 확대 유지 / 무대 두 손가락 핀치 유지 / `.dz-body` 의 `touch-action` 이 `pan-y`.

### 3.2.36 잔털 지우개를 제대로 (2026-09-13)

연구자: "잔털 지우개가 잘 안 되는 것 같아." 측정해 보니 세 가지가 겹쳐 있었음.

1. **밝기로 털을 찾을 수 없다.** 연구자 사진에서 실측(D = 국소배경 − 밝기): 잔털 부위 `p90 = 17`, **깨끗한 볼 피부 `p90 = 24`**. 즉 피부 결(모공)이 털보다 대비가 크다. 기존 구현은 `D > 10` 을 털로 보고 지웠으므로 **모공까지 같이 지워져 뭉갠 자국**이 남았음(grain 5.07 → 2.2).
2. **모양으로 찾는 것도 안 된다.** 연결성분 + 길이/가늘기 필터로 바꿔 봤으나, 눈썹 가장자리처럼 털이 서로 붙으면 89×47 짜리 한 덩어리가 되어 임계값에 따라 **눈썹 전체를 지우거나(31%) 아무것도 안 지우거나(0.3%)** 로 튐. 예측 불가.
3. **손을 뗄 때 한꺼번에 적용**되어 칠하는 동안 아무 일도 안 일어남 — "안 먹는다"는 느낌의 큰 원인.

**바꾼 방식** — 포토샵 스팟 힐링과 같은 생각: *무엇이 털인지 판정하지 않는다. 브러시가 곧 판정이다.* 붓 자국 아래를 **주변의 깨끗한 피부로 옮겨 붙인다**(`healStamp`).
- 후보: 반경 1.7·2.3·3.0 r 의 16방향 = 48개. 각 후보에 대해 (a) 가져올 피부의 어두운 구조 에너지, (b) 주변 링과의 색 차, (c) **이미 지운 자리 비율**(복사의 복사를 막아 결이 흐려지지 않게).
- 2단계 선택: 먼저 **가장 깨끗한 것 기준 1.5배 + 12 이내**만 남기고, 그 안에서 색·신선도로 고름. (한 번에 가중합으로 고르면 "깨끗함"이 밀려 털을 다시 붙여 옴.)
- **톤 맞춤은 붓 자국이 아니라 그 바깥 링(r~1.6r)에서** 계산. 자국 안에는 지우려는 털이 들어 있어 그 평균에 맞추면 **붙인 피부가 어두워져 자국이 그대로 보였음**(실제로 그랬음).
- 가장자리 페더 0.4r~r, 한 점당 1~3ms → **움직이는 동안 dab 마다 즉시 적용**(0.34 r 간격). 되돌리기는 획 단위, 재생은 같은 순서로 같은 결과(테스트로 픽셀 동일성 고정).
- 실측: 잔털 띠에서 dark 1.27% → 한 번 긋고 0.73%, 세 번 긋고 **0.25%**, grain 4.78 → **3.63**(예전 방식은 2.2 로 뭉갬). 기본 지름 1.5 → **2.0 mm**.
- 검증 10항목(첫 터치에 바로 반응 / 긋는 도중 지워짐 / 1회·3회 제거율 / 결 보존 / 41 dab 1초 / 되돌리기 / 재생 픽셀 동일).

### 3.2.37 꼬리 쪽 조절점 추가 · 미세조정이 눈썹을 찌그러뜨리던 버그 (2026-09-13)

- 연구자: "꼬리 쪽에 위아래로 하나씩 더. 간격이 성겨서." 맞음 — 아치(u≈0.5)에서 꼬리(u=1)까지가 한 구간이라 그 사이를 잡을 방법이 없었음.
- 도안 샘플 `iK = (iA+N)/2` 를 기준으로 **`tT`/`tB`** 추가 → 눈썹당 **9개**(머리·중간·아치·꼬리앞 × 윗/아랫 + 꼬리). 변위 보간 매듭이 `[uH,uM,uA,1]` → `[uH,uM,uA,uK,1]`. 실측 간격: 윗선 -0.03 / 0.21 / 0.42 / 0.71 / 1.0 — 가장 넓은 틈 0.29.
- 예전에 저장한 상담은 없는 조절점을 **곡선이 이미 지나는 자리**에 만들어 붙임(`ensureMid`, 그리고 `browGeometry` 의 `if(!b.tT) dT[3]=…`). 5점 시절 상태를 넣고 열어 9점이 되는지 테스트로 고정.
- **버그**: `nudge()` 의 `all=['hT','hB','aT','aB','t']` — 중간 점이 빠져 있어 `▲위로/▼아래로/◀안쪽/▶바깥` 이 **눈썹을 통째로 옮기지 않고 일부만 옮겨 찌그러뜨리고 있었음**. `Object.keys(b)` 로 고침(테스트: 9개가 전부 같은 거리만큼 이동).
- **작업 방식 교훈**: 한 파이썬 패치 스크립트에서 `rep()` 하나가 실패하면 `open(...,'w')` 에 도달하지 못해 **그 스크립트의 모든 치환이 롤백**된다. 이번에 실제로 그래서 "적용됐다고 생각했는데 파일엔 없는" 상태가 한 번 나왔음(테스트가 바로 잡음). 패치 후에는 반드시 `grep` 으로 실제 반영을 확인할 것.
- 검증: 조절점 7항목(9개 존재·간격·꼬리앞 점이 아치를 건드리지 않고 윤곽을 들어올림·대칭·전체 이동·구버전 승격), 잔털 9항목(붓이 지나간 자리 1.8% → **0.03%**), e2e 44, 그 외 전 스위트 통과.

### 3.2.38 "메뉴가 자꾸 확대된다" — 진짜 원인은 입력칸 글자 크기 (2026-09-13)

- 3.2.35 에서 두 손가락 핀치는 막았는데도 연구자가 "자꾸 메뉴가 확대된다"고 다시 보고. 핀치가 아니었음.
- **iOS/iPadOS 는 글자 크기가 16px 미만인 `input`/`select`/`textarea` 에 포커스가 가면 그 칸이 읽힐 만큼 페이지 전체를 확대**한다(접근성 동작, 끌 수 없음). 그리고 **스스로 되돌아가지 않는다** — 한 번 탭하면 확대된 채로 남는다. 해당되는 것들: `#dzClient` 14px, `#dzSide` 13px(인라인), 그리고 **1120px 이상 화면의 `.field input/select/textarea` 13px** — 아이패드 가로가 여기 걸림.
- 고침: `@media (pointer:coarse)` 로 **손가락 입력 기기에서만** 타이핑 가능한 컨트롤을 16px 로. 마우스 환경은 기존 크기 유지(데스크톱 13px 확인).
  - **이 블록은 스타일시트 맨 끝에 있어야 한다.** 처음엔 중간에 뒀는데 뒤에 오는 `@media (min-width:1120px)` 규칙이 13px 로 되돌려 놓아 **아이패드 가로에서만** 여전히 확대됐다(세로·폰은 통과, 가로만 실패 — 이걸 못 봤으면 "고쳤는데 안 고쳐졌다"가 될 뻔).
  - `#dzSide` 는 인라인 `style="font-size:13px"` 이라 CSS 로는 못 이김 → 인라인 제거.
- 더불어 `gesturestart` 계열을 캡처 단계로 올리고, `touchmove` 에 손가락 2개 이상이면 `preventDefault`(스튜디오가 열려 있는 동안만).
- 검증: 아이패드 가로/세로·폰 각각에서 **화면에 보이는 모든 타이핑 컨트롤의 계산된 글자 크기**를 재서 16px 미만이 하나도 없음을 확인(스튜디오 4탭 + 메뉴 4개), 마우스 환경은 13px 유지.
